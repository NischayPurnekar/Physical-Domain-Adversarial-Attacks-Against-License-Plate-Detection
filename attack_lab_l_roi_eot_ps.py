...
using ground-truth Pascal VOC XML bounding boxes, with:

- Lab-L-only perturbations
- center-ROI character exclusion masking
- EOT-based optimization
- optional differentiable print-scan simulation
- epsilon warmup
- finite-value guards
- EOT-based early stopping
- final robustness evaluation
- optional saving of luminance perturbations

Overview
--------
This script:
- reads GT bounding boxes from XML annotations
- letterboxes images to detector input size while preserving resize/padding metadata
- converts images to Lab and perturbs only the L channel
- keeps a,b channels fixed
- excludes darker character regions only inside a center ROI of the plate
- keeps RGB pixels exactly clean outside the GT bbox
- minimizes a score-aligned differentiable proxy derived from YOLO raw outputs
- applies EOT during optimization
- optionally applies a differentiable CycleGAN-based print-scan simulator inside EOT
- supports epsilon warmup to reduce early saturation
- supports finite-value guards and alpha decay on repeated non-finite gradients
- supports EOT-based early stopping using robustness success
- also includes the requested hard-coded early stop at detector confidence 0.05
- performs final robustness evaluation under the same EOT distribution
- saves per-image outputs, summaries, and CSV files

Important compatibility note
----------------------------
This script assumes that the raw forward pass of the loaded YOLOv8 model returns
a tuple in which `raw[1]["scores"]` exists and is compatible with shape `[B, 1, N]`.
This depends on the specific Ultralytics version used in the experiments.
Please pin the Ultralytics version in your environment for reproducibility.

Lab-L perturbation note
-----------------------
The CLI uses `--eps` and `--alpha` in an RGB-style normalized [0,1] scale.
Internally, both are converted to Lab L-channel units by multiplying by 100.

Print-scan simulator note
-------------------------
If `--use_ps` is enabled, this script expects a compatible `networks.py` module
to be available on the Python path and a valid CycleGAN generator checkpoint.
"""
...
def safe_lab_to_rgb(lab: torch.Tensor) -> torch.Tensor:
    rgb = kc.lab_to_rgb(lab)
    rgb = torch.nan_to_num(rgb, nan=0.0, posinf=1.0, neginf=0.0)
    return rgb.clamp(0.0, 1.0)


def validate_inputs(weights: str, source_dir: str, xml_dir: str, use_ps: bool, ps_model_path: str):
    ...
    if use_ps and not Path(ps_model_path).exists():
        raise FileNotFoundError(f"Print-scan model checkpoint not found: {ps_model_path}")
...
# =================== Luminance perturbation saving =================== #
def save_deltaL_artifacts(
    out_dir: str,
    deltaL: torch.Tensor,
    ...
    d = (deltaL * plate_mask).detach().float().cpu().numpy()[0, 0]
    npy_path = os.path.join(out_dir, f"{tag}_iter_{t:04d}.npy")
    np.save(npy_path, d.astype(np.float32))

    denom = np.max(np.abs(d)) + 1e-12
    vis = d / denom
    vis = (vis * 127.0 + 128.0).clip(0, 255).astype(np.uint8)
    png_path = os.path.join(out_dir, f"{tag}_iter_{t:04d}.png")
    save_gray(png_path, vis)


# ========================= Differentiable CycleGAN Print-&-Scan ========================= #
class DifferentiableCycleGAN(nn.Module):
    """
    CycleGAN generator wrapper used as a differentiable print-scan simulator.
    """
    ...
    def __init__(
        ...
        gpu_ids = "0" if self.device.type == "cuda" else ""

        self.model = networks.define_G(
            3, 3, 64, "unet_256", "instance",
        ...
        self.resize_down = T.Resize(down_hw, interpolation=T.InterpolationMode.BILINEAR)
        self.resize_up = T.Resize(out_hw, interpolation=T.InterpolationMode.BILINEAR)
        self.norm = T.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
        self.add_noise_std = float(add_noise_std)

    def set_out_hw(self, out_hw):
        self.resize_up = T.Resize(tuple(out_hw), interpolation=T.InterpolationMode.BILINEAR)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.add_noise_std > 0:
            y = y + self.add_noise_std * torch.randn_like(y)

        y = self.resize_up(y)
        y = safe01(y)
        return y.to(x0.device)


# ========================= EOT ========================= #
class EOTWrapper:
    """
    Differentiable EOT wrapper for physical-like variation.
    Returns [N, 3, H, W] in [0,1].

    Optionally includes a differentiable CycleGAN print-scan simulator.
    """
    ...
    def __init__(
        ...
        self.use_ps = bool(use_ps)
        self.ps_model_path = str(ps_model_path) if ps_model_path is not None else ""
        self.ps_down_hw = tuple(ps_down_hw)
        self.ps_noise_std = float(ps_noise_std)

        self.ps_simulator = None
        if self.use_ps:
            if (not self.ps_model_path) or (not os.path.exists(self.ps_model_path)):
                raise FileNotFoundError(f"--use_ps set but --ps_model_path not found: {self.ps_model_path}")

            dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            self.ps_simulator = DifferentiableCycleGAN(
                model_path=self.ps_model_path,
                out_hw=(640, 640),
                down_hw=self.ps_down_hw,
                device=dev,
                add_noise_std=self.ps_noise_std,
            )
    ...
    def apply(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_ps and self.ps_simulator is not None:
            self.ps_simulator.set_out_hw((H, W))
        ...
        for _ in range(self.n):
            ...
            if self.use_ps and self.ps_simulator is not None:
                y = self.ps_simulator(img.unsqueeze(0)).squeeze(0)
                if y.shape[-2:] != (H, W):
                    y = TF.resize(y, size=[H, W], antialias=True)
                img = safe01(y)
...
def make_center_roi_mask(imgsz: int, xyxy, margin: int, shrink: float, device: str):
    """
    Returns [1,1,imgsz,imgsz] mask for a center ROI inside the bbox.
    shrink=0.2 trims 20% from each side, keeping the central 60%.
    """
    x1, y1, x2, y2 = map(float, xyxy)
    x1 = max(0.0, x1 - float(margin))
    y1 = max(0.0, y1 - float(margin))
    x2 = min(float(imgsz), x2 + float(margin))
    y2 = min(float(imgsz), y2 + float(margin))
    ...
    rx1 = int(max(0.0, x1 + dx))
    ry1 = int(max(0.0, y1 + dy))
    rx2 = int(min(float(imgsz), x2 - dx))
    ry2 = int(min(float(imgsz), y2 - dy))
...
def topk_score_loss_from_raw(raw, topk: int, clamp_logits: float = 20.0):
    """
    Minimize mean(top-k sigmoid(scores)) to suppress detections.
    Expects raw[1]["scores"] with shape [B,1,N] or reshape-compatible.
    """
    ...
    s = s.clamp(-clamp_logits, clamp_logits)
    p = s.sigmoid()

    loss = torch.topk(p, k).values.mean()
...
@torch.no_grad()
def eval_robust_success_quick(
    model_yolo: YOLO,
    eot: EOTWrapper,
    adv_letterboxed_01: torch.Tensor,
    meta: dict,
    ul_device,
    conf: float,
    iou: float,
):
    if not (adv_letterboxed_01.ndim == 4 and adv_letterboxed_01.shape[0] == 1):
        raise ValueError("adv_letterboxed_01 must have shape [1,3,H,W]")

    adv_eot = eot.apply(adv_letterboxed_01)
    n = int(adv_eot.shape[0])
    if n <= 0:
        return 0.0, 0.0
    ...
    for i in range(n):
        letter_i = tensor_to_rgb_uint8(adv_eot[i:i + 1])
        orig_i = reconstruct_orig_from_letterboxed_rgb(letter_i, meta)
        if detector_has_detections(model_yolo, orig_i, ul_device, conf, iou):
            det += 1

    det_rate = float(det) / float(n)
    return det_rate, 1.0 - det_rate


# ========================= MAIN ========================= #
def main():
    ap = argparse.ArgumentParser(
        description="Lab-L-only masked I-FGSM attack with center-ROI character exclusion, optional print-scan simulation, EOT, and robustness evaluation for YOLOv8 license-plate detection."
    )
    ...
    ap.add_argument("--out_dir", type=str, default="outputs/adv_lab_l_roi_eot_ps", help="Directory where outputs will be saved.")

    ap.add_argument("--imgsz", type=int, default=640, help="Letterbox image size used for the detector.")
    ap.add_argument("--eps", type=float, default=48 / 255,
                    help="Maximum perturbation budget in normalized [0,1] scale; internally converted to Lab-L units via *100.")
    ap.add_argument("--steps", type=int, default=2000, help="Number of attack optimization steps.")
    ap.add_argument("--alpha", type=float, default=4 / 255,
                    help="Step size in normalized [0,1] scale; internally converted to Lab-L units via *100.")

    ap.add_argument("--eps_warmup_steps", type=int, default=100,
                    help="Linearly increase eps from 0 to eps over the first K iterations.")
    ap.add_argument("--nan_patience", type=int, default=10,
                    help="Number of non-finite gradient steps tolerated before reducing alpha.")
    ap.add_argument("--alpha_decay_on_nan", type=float, default=0.5,
                    help="Factor used to reduce alpha after repeated non-finite gradients.")
    ap.add_argument("--min_alpha", type=float, default=0.25 / 255,
                    help="Lower bound for alpha after decay, expressed in normalized [0,1] scale.")

    ap.add_argument("--margin", type=int, default=10, help="Extra margin around GT bbox in letterbox space (pixels).")
    ap.add_argument("--topk", type=int, default=600, help="Top-k raw candidate scores used in the differentiable suppression loss.")
    ...
    ap.add_argument("--earlystop_conf", type=float, default=0.25, help="Confidence threshold used in detector-based early-stop checks.")
    ap.add_argument("--earlystop_untransformed", action=argparse.BooleanOptionalAction, default=False,
                    help="Whether to allow early stopping when the untransformed adversarial original has no detections.")
    ap.add_argument("--earlystop_eot", action=argparse.BooleanOptionalAction, default=False,
                    help="Whether to enable EOT-based early stopping using robustness success.")
    ap.add_argument("--earlystop_eot_target", type=float, default=0.60,
                    help="Stop when robust_success(no-det) reaches or exceeds this target.")
    ap.add_argument("--earlystop_eot_n", type=int, default=50,
                    help="Number of EOT transforms used in the quick EOT early-stop check.")
    ap.add_argument("--earlystop_eot_every", type=int, default=10,
                    help="Evaluate EOT early-stop every K iterations.")
    ap.add_argument("--earlystop_eot_min_iter", type=int, default=0,
                    help="Do not evaluate EOT early-stop before this iteration index.")

    ap.add_argument("--device", type=str, default="auto", help="Computation device: auto, cpu, cuda, cuda:0, etc.")

    ap.add_argument("--save_plate_patch", action=argparse.BooleanOptionalAction, default=False,
                    help="Whether to save cropped clean/adversarial plate patches.")
    ap.add_argument("--plate_patch_margin", type=int, default=10,
                    help="Extra crop margin for saved plate patches at original resolution.")
    ap.add_argument("--plate_out_w", type=int, default=4094,
                    help="Optional output width for resized printable adversarial patch. Set <=0 to disable.")
    ap.add_argument("--plate_out_h", type=int, default=866,
    ...
    ap.add_argument("--gt_mask_margin", type=int, default=0,
                    help="Margin in original-resolution space for saved overlays/masks.")
    ap.add_argument("--save_adv_original_dir", action=argparse.BooleanOptionalAction, default=True,
                    help="Whether to save all adversarial original-resolution images into a flat output folder.")

    ap.add_argument("--debug_heatmap", action=argparse.BooleanOptionalAction, default=False,
                    help="Whether to save optional score heatmaps during optimization.")
    ap.add_argument("--debug_heatmap_every", type=int, default=50,
                    help="Save a debug heatmap every N iterations.")

    ap.add_argument("--char_thresh_L", type=float, default=50.0,
                    help="Character mask threshold in Lab-L units. Character mask is defined as (L < threshold).")
    ap.add_argument("--save_char_mask", action=argparse.BooleanOptionalAction, default=True,
                    help="Whether to save character-mask and attack-mask visualizations.")

    ap.add_argument("--save_lum_pert", action=argparse.BooleanOptionalAction, default=True,
                    help="Whether to save luminance perturbations during and after the attack.")
    ap.add_argument("--save_lum_every", type=int, default=50,
                    help="Save deltaL artifacts every N iterations.")
    ap.add_argument("--save_lum_tag", type=str, default="deltaL",
                    help="Filename prefix used for saved deltaL artifacts.")

    # EOT args
    ap.add_argument("--eot_n", type=int, default=15, help="Number of EOT transforms per optimization step.")
    ...
    ap.add_argument("--eot_translate", type=float, default=0.05, help="Maximum translation fraction relative to image size.")
    ap.add_argument("--eot_d_ref", type=float, default=0.9, help="Reference distance parameter used to derive scale.")
    ap.add_argument("--eot_d_min", type=float, default=0.9, help="Minimum sampled distance parameter.")
    ap.add_argument("--eot_d_max", type=float, default=1.35, help="Maximum sampled distance parameter.")
    ap.add_argument("--eot_perspective", type=float, default=0.004, help="Perspective distortion magnitude.")
    ap.add_argument("--eot_shear", type=float, default=2.5, help="Maximum shear angle.")
    ap.add_argument("--eot_p_blur", type=float, default=0.3, help="Probability of blur.")
    ap.add_argument("--eot_motion_blur_max", type=int, default=13, help="Maximum motion-blur kernel size.")
    ap.add_argument("--eot_gauss_sigma_min", type=float, default=0.3, help="Minimum Gaussian blur sigma.")
    ap.add_argument("--eot_gauss_sigma_max", type=float, default=2.5, help="Maximum Gaussian blur sigma.")
    ap.add_argument("--eot_p_illum", type=float, default=0.30, help="Probability of illumination perturbation.")
    ...
    ap.add_argument("--eot_p_resample", type=float, default=0.4, help="Probability of down/up resampling.")
    ...
    # P&S args
    ap.add_argument("--use_ps", action=argparse.BooleanOptionalAction, default=False,
                    help="Whether to enable the CycleGAN print-scan simulator inside EOT.")
    ap.add_argument("--ps_model_path", type=str, default="weights/ps_generator.pth",
                    help="Path to the CycleGAN generator checkpoint.")
    ap.add_argument("--ps_down_hw", type=int, nargs=2, default=[256, 256],
                    help="Downsample size before CycleGAN inference: H W.")
    ap.add_argument("--ps_noise_std", type=float, default=0.0005,
                    help="Small noise added after CycleGAN output. Set 0 to disable.")

    ap.add_argument("--save_eot_samples", action=argparse.BooleanOptionalAction, default=False,
                    help="Whether to save some EOT-transformed samples during optimization.")
    ap.add_argument("--save_eot_every", type=int, default=25,
                    help="Save EOT samples every N iterations.")
    ap.add_argument("--eot_save_max_per_iter", type=int, default=8,
                    help="Maximum number of EOT samples to save per selected iteration.")

    ap.add_argument("--robust_eval", action=argparse.BooleanOptionalAction, default=True,
                    help="Whether to evaluate final robustness under the same EOT distribution.")
    ap.add_argument("--robust_n", type=int, default=50,
                    help="Number of EOT samples used for final robustness evaluation.")
    ap.add_argument("--robust_conf", type=float, default=0.25,
                    help="Confidence threshold used during robustness evaluation.")
    ap.add_argument("--robust_iou", type=float, default=0.5,
                    help="IoU threshold used during robustness evaluation.")
    ap.add_argument("--save_robust_samples", action=argparse.BooleanOptionalAction, default=True,
                    help="Whether to save some transformed robustness samples and detection plots.")
    ap.add_argument("--robust_save_max", type=int, default=50,
                    help="Maximum number of robustness samples to save per image.")
    ...
    validate_inputs(args.weights, args.source_dir, args.xml_dir, args.use_ps, args.ps_model_path)
    ...
    eps_L = float(args.eps) * 100.0
    alpha_base_rgb = float(args.alpha if args.alpha is not None else 2.0 * (args.eps / max(args.steps, 1)))
    alpha_L_base = alpha_base_rgb * 100.0
    min_alpha_L = float(args.min_alpha) * 100.0
    alpha_L = alpha_L_base
    ...
    eot = EOTWrapper(
        ...
        use_ps=args.use_ps,
        ps_model_path=args.ps_model_path,
        ps_down_hw=tuple(args.ps_down_hw),
        ps_noise_std=args.ps_noise_std,
    )

    eot_early = EOTWrapper(
        n=args.earlystop_eot_n,
        p_flip=args.eot_p_flip,
        degrees=args.eot_degrees,
        translate=args.eot_translate,
        d_ref=args.eot_d_ref,
    ...
    lum_root = os.path.join(args.out_dir, "luminance_perturbations")
    if args.save_lum_pert:
        ensure_dir(lum_root)
    ...
    robust_rows = [("filename", "det_rate_eot", "robust_success_eot", "robust_n", "robust_conf", "robust_iou", "use_ps")]
    ...
    for img_path in img_files:
        ...
        mask3_bbox = bbox_mask.repeat(1, 3, 1, 1)
        roi = make_center_roi_mask(args.imgsz, bbox_l, args.margin, ROI_SHRINK, device)
        ...
        x0_lab = kc.rgb_to_lab(x0)
        x0_lab = torch.nan_to_num(x0_lab, nan=0.0, posinf=0.0, neginf=0.0)
        L0 = x0_lab[:, 0:1].detach()
        a0 = x0_lab[:, 1:2].detach()
        b0 = x0_lab[:, 2:3].detach()

        char_mask = (L0 < float(args.char_thresh_L)).float()
        char_mask = char_mask * roi
        plate_mask = bbox_mask * (1.0 - char_mask)

        if args.save_char_mask:
            save_gray(os.path.join(per_dir, "char_mask.png"),
                      (char_mask[0, 0].detach().cpu().numpy() * 255).astype(np.uint8))
            save_gray(os.path.join(per_dir, "plate_attack_mask.png"),
                      (plate_mask[0, 0].detach().cpu().numpy() * 255).astype(np.uint8))
            save_gray(os.path.join(per_dir, "roi_center.png"),
                      (roi[0, 0].detach().cpu().numpy() * 255).astype(np.uint8))

        deltaL = torch.zeros_like(L0, device=device).requires_grad_(True)
        best_adv, early_stopped = None, False
        nan_counter = 0
        alpha_L = alpha_L_base
        best_seen_success = -1.0

        lum_dir_img = os.path.join(lum_root, stem)
        if args.save_lum_pert:
            ensure_dir(lum_dir_img)

        try:
            for t in range(args.steps):
                if args.eps_warmup_steps > 0:
                    frac = min(1.0, float(t + 1) / float(args.eps_warmup_steps))
                    eps_t_L = eps_L * frac
                else:
                    eps_t_L = eps_L

                L_adv = (L0 + deltaL * plate_mask).clamp(0.0, 100.0)
                lab_adv = torch.cat([L_adv, a0, b0], dim=1)
                rgb_adv_full = safe_lab_to_rgb(lab_adv)

                adv = x0 * (1.0 - mask3_bbox) + rgb_adv_full * mask3_bbox
                adv = safe01(adv)
                ...
                if args.save_lum_pert and args.save_lum_every > 0 and (t % args.save_lum_every == 0):
                    save_deltaL_artifacts(
                        out_dir=lum_dir_img,
                        deltaL=deltaL,
                        plate_mask=plate_mask,
                        t=t,
                        tag=args.save_lum_tag,
                    )
                ...
                Ls = torch.stack(per_losses)
                m = max(1, int(round(0.33 * Ls.numel())))
                loss_top = torch.topk(Ls, m).values.mean()
                loss_mean = Ls.mean()
                loss = 0.85 * loss_top + 0.15 * loss_mean
                loss = torch.nan_to_num(loss, nan=0.0, posinf=1e3, neginf=-1e3)

                if scores_for_debug is not None:
                    mv, mi = scores_for_debug.max(dim=0)
                    mv_prob = torch.nan_to_num(mv, nan=0.0, posinf=0.0, neginf=0.0).sigmoid()
                else:
                    mv = torch.tensor(0.0, device=device)
                    mi = torch.tensor(0, device=device)
                    mv_prob = torch.tensor(0.0, device=device)

                if (t % max(args.log_every, 1) == 0) or (t == args.steps - 1):
                    lmin = float(torch.nan_to_num(Ls.min(), nan=0.0).detach().cpu().item())
                    lmax = float(torch.nan_to_num(Ls.max(), nan=0.0).detach().cpu().item())
                    with torch.no_grad():
                        sat = (deltaL.abs() >= (eps_t_L - 1e-12)).float().mean().item() * 100.0
                        dinf = float(deltaL.abs().max().detach().cpu().item())
                    print(
                        f"  iter {t:04d} | eps_t_L={eps_t_L:.3f} alpha_L={alpha_L:.3f} | "
                        f"EOT loss={loss.item():.6f} | min={lmin:.6f} max={lmax:.6f} | "
                        f"max_logit={mv.item():.3f} prob={mv_prob.item():.4f} idx={int(mi.item())}"
                    )
                    print(f"    deltaL | Linf(L-units)={dinf:.4f} / eps_t_L={eps_t_L:.4f} | sat%={sat:.2f}")

                # Hard-coded early stop at conf=0.05, as requested
                adv_orig_rgb = reconstruct_orig_from_letterboxed_rgb(tensor_to_rgb_uint8(adv), meta)
                if not detector_has_detections(model, adv_orig_rgb, ul_device, 0.05, args.iou_det):
                ...
                if (
                    args.earlystop_eot
                    and (t >= int(args.earlystop_eot_min_iter))
                    and (args.earlystop_eot_every > 0)
                    and (t % int(args.earlystop_eot_every) == 0)
                ):
                    det_rate_q, success_q = eval_robust_success_quick(
                        model_yolo=model,
                        eot=eot_early,
                        adv_letterboxed_01=adv.detach(),
                        meta=meta,
                        ul_device=ul_device,
                        conf=args.earlystop_conf,
                        iou=args.iou_det,
                    )
                    best_seen_success = max(best_seen_success, float(success_q))
                    print(f"    [EARLY-EOT] det_rate={det_rate_q:.3f} | robust_success={success_q:.3f} (N={args.earlystop_eot_n}) | best={best_seen_success:.3f}")

                    if float(success_q) >= float(args.earlystop_eot_target):
                        print(f"  [✓] Early stop (EOT robust_success >= {args.earlystop_eot_target:.2f}) at iter {t}")
                        early_stopped = True
                        best_adv = adv.detach()
                        break

                if args.earlystop_untransformed:
                    adv_orig_rgb2 = reconstruct_orig_from_letterboxed_rgb(tensor_to_rgb_uint8(adv), meta)
                    if not detector_has_detections(model, adv_orig_rgb2, ul_device, args.earlystop_conf, args.iou_det):
                        print(f"  [✓] Early stop (untransformed) at iter {t}")
                        early_stopped = True
                        best_adv = adv.detach()
                        break

                net.zero_grad(set_to_none=True)
                if deltaL.grad is not None:
                    deltaL.grad.zero_()
                ...
                if (g is None) or (not torch.isfinite(g).all()):
                    nan_counter += 1
                    print(f"  [!] Non-finite grad at iter {t} (count={nan_counter}). Skipping update.")
                    if nan_counter >= args.nan_patience:
                        alpha_L = max(alpha_L * args.alpha_decay_on_nan, min_alpha_L)
                        nan_counter = 0
                        print(f"  [!] Reducing alpha_L -> {alpha_L:.6f} due to repeated non-finite gradients.")
                    best_adv = adv.detach()
                    with torch.no_grad():
                        deltaL = torch.nan_to_num(deltaL, nan=0.0, posinf=0.0, neginf=0.0).detach().requires_grad_(True)
                    continue
                else:
                    nan_counter = 0
                ...
                if (t % max(args.log_every, 1) == 0) or (t == args.steps - 1):
                    with torch.no_grad():
                        ...
                        print(f"    gradL | mean={gm:.3e} max={gmx:.3e} nonzero%={nz:.2f}")
                ...
                with torch.no_grad():
                    deltaL = (deltaL - alpha_L * g.sign()).clamp(-eps_t_L, eps_t_L)
                    deltaL = deltaL * plate_mask
                    deltaL = torch.nan_to_num(deltaL, nan=0.0, posinf=0.0, neginf=0.0).detach().requires_grad_(True)
        ...
        if best_adv is None:
            ...
            lab_adv = torch.cat([L_adv, a0, b0], dim=1)
            rgb_adv_full = safe_lab_to_rgb(lab_adv)
            best_adv = (x0 * (1.0 - mask3_bbox) + rgb_adv_full * mask3_bbox).detach()
        ...
        if args.save_lum_pert:
            save_deltaL_artifacts(
                ...
                plate_mask=plate_mask,
                t=args.steps,
                tag=f"{args.save_lum_tag}_final",
            )
        ...
        with open(os.path.join(per_dir, "summary.txt"), "w", encoding="utf-8") as f:
            ...
            f.write(f"eps(rgb-scale): {args.eps} -> eps_L: {eps_L}\n")
            f.write(f"alpha(rgb-scale): {alpha_base_rgb} -> alpha_L_base: {alpha_L_base}\n")
            f.write(f"eps_warmup_steps: {args.eps_warmup_steps}\n")
            ...
            f.write("\n[ROI center]\n")
            f.write(f"ROI_SHRINK: {ROI_SHRINK}\n")

            f.write("\n[NaN handling]\n")
            f.write(f"nan_patience: {args.nan_patience}\n")
            f.write(f"alpha_decay_on_nan: {args.alpha_decay_on_nan}\n")
            f.write(f"min_alpha(rgb-scale): {args.min_alpha} -> min_alpha_L: {min_alpha_L}\n")

            f.write("\n[L-CHANNEL + CHAR EXCLUSION]\n")
            f.write(f"char_thresh_L: {args.char_thresh_L}\n")
            f.write(f"save_char_mask: {args.save_char_mask}\n")
            f.write(f"save_lum_pert: {args.save_lum_pert}\n")
            f.write(f"save_lum_every: {args.save_lum_every}\n")
            f.write(f"save_lum_tag: {args.save_lum_tag}\n")
            ...
            f.write("\n[EARLYSTOP_EOT]\n")
            f.write(f"earlystop_eot: {args.earlystop_eot}\n")
            f.write(f"earlystop_eot_target: {args.earlystop_eot_target}\n")
            f.write(f"earlystop_eot_n: {args.earlystop_eot_n}\n")
            f.write(f"earlystop_eot_every: {args.earlystop_eot_every}\n")
            f.write(f"earlystop_eot_min_iter: {args.earlystop_eot_min_iter}\n")
            f.write(f"earlystop_untransformed: {args.earlystop_untransformed}\n")

            f.write("\n[P&S]\n")
            f.write(f"use_ps: {args.use_ps}\n")
            f.write(f"ps_model_path: {args.ps_model_path}\n")
            f.write(f"ps_down_hw: {tuple(args.ps_down_hw)}\n")
            f.write(f"ps_noise_std: {args.ps_noise_std}\n")
    ...
    if args.save_lum_pert:
        print(f"Luminance perturbations saved in: {lum_root}")
    if args.robust_eval and args.save_robust_samples:
