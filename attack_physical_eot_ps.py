...
optional differentiable print-scan simulation, and post-attack robustness evaluation.
...
- supports epsilon warmup to reduce early saturation of the perturbation budget
- includes finite-value guards for tensors, losses, and gradients
...
- optional print-scan simulation is applied inside EOT
- optional early-stop robustness checks use additional transformed samples
- final robustness evaluation uses further transformed samples
"""
...
def safe01(x: torch.Tensor) -> torch.Tensor:
    """
    Convert NaN/Inf to finite values and clamp into [0,1].
    """
    x = torch.nan_to_num(x, nan=0.0, posinf=1.0, neginf=0.0)
    return x.clamp(0.0, 1.0)


def ensure_finite(x: torch.Tensor, name: str = "tensor") -> bool:
...
class DifferentiableCycleGAN(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        import networks  # must exist on the Python path
...
def main():
    ap = argparse.ArgumentParser(
        description="Masked I-FGSM adversarial attack with EOT, optional print-scan simulation, and robustness evaluation for YOLOv8 license-plate detection."
    )
    ...
    ap.add_argument("--out_dir", type=str, default="outputs/adv_ifgsm_eot_ps", help="Directory where outputs will be saved.")

    ap.add_argument("--imgsz", type=int, default=640, help="Letterbox image size used for the detector.")
    ap.add_argument("--eps", type=float, default=108 / 255, help="Maximum Linf perturbation budget in [0,1] scale.")
    ap.add_argument("--steps", type=int, default=2000, help="Number of attack optimization steps.")
    ap.add_argument("--alpha", type=float, default=12 / 255, help="Step size per iteration.")
    ...
    ap.add_argument("--min_alpha", type=float, default=0.25 / 255,
                    help="Lower bound for alpha after decay.")
    ...
    # P&S args
    ap.add_argument("--use_ps", action=argparse.BooleanOptionalAction, default=True,
    ...
    ap.add_argument("--robust_save_max", type=int, default=5,
    ...
    alpha_base = float(args.alpha if args.alpha is not None else 2.0 * (args.eps / max(args.steps, 1)))
    alpha = alpha_base
    ...
    for img_path in img_files:
        ...
        try:
            for t in range(args.steps):
                ...
                m = max(1, int(round(0.33 * L.numel())))
                loss_top = torch.topk(L, m).values.mean()
                loss_mean = L.mean()
                ...
                if (t % max(args.log_every, 1) == 0) or (t == args.steps - 1):
                    lmin = float(torch.nan_to_num(L.min(), nan=0.0).detach().cpu().item())
                    lmax = float(torch.nan_to_num(L.max(), nan=0.0).detach().cpu().item())
                    print(
                        f"  iter {t:04d} | eps_t={eps_t:.5f} alpha={alpha:.5f} | "
                        f"EOT loss={loss.item():.6f} | min={lmin:.6f} max={lmax:.6f} | "
                        f"max_logit={mv.item():.3f} prob={mv_prob.item():.4f} idx={int(mi.item())}"
                    )

                if args.earlystop_eot and (t >= int(args.earlystop_eot_min_iter)) and (args.earlystop_eot_every > 0) and (t % int(args.earlystop_eot_every) == 0):
                ...
                if (g is None) or (not torch.isfinite(g).all()):
                    ...
                    if nan_counter >= args.nan_patience:
                        alpha = max(alpha * args.alpha_decay_on_nan, args.min_alpha)
                        nan_counter = 0
                        print(f"  [!] Reducing alpha -> {alpha:.6f} due to repeated non-finite gradients.")
                    best_adv = adv.detach()
                    with torch.no_grad():
                        delta = torch.nan_to_num(delta, nan=0.0, posinf=0.0, neginf=0.0).detach().requires_grad_(True)
                    continue
                else:
                    nan_counter = 0
                ...
                if (t % max(args.log_every, 1) == 0) or (t == args.steps - 1):
                    with torch.no_grad():
                        ...
                        sat = (delta.abs() >= (eps_t - 1e-12)).float().mean().item() * 100.0
                ...
                with torch.no_grad():
                    delta = (delta - alpha * g.sign()).clamp(-eps_t, eps_t)
                    delta = delta * mask3
                    delta = torch.nan_to_num(delta, nan=0.0, posinf=0.0, neginf=0.0).detach().requires_grad_(True)
        ...
        with open(os.path.join(per_dir, "summary.txt"), "w", encoding="utf-8") as f:
            f.write(f"image: {img_path.name}\n")
            f.write(f"steps: {args.steps}, eps: {args.eps}, alpha_base: {alpha_base}\n")
            ...
            f.write(f"min_alpha: {args.min_alpha}\n")
