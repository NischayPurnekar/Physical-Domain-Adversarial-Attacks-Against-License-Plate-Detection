...
- character exclusion masking
- EOT-based optimization
- post-attack robustness evaluation
...
- excludes darker character regions from the attack mask using a threshold on L
...
- optionally saves luminance perturbations (deltaL) as .npy and visualization .png
...
-----------------------
The CLI uses `--eps` and `--alpha` in an RGB-style normalized [0,1] scale for convenience.
Internally, both are converted to Lab L-channel units by multiplying by 100, because
Lab L approximately lies in [0,100].

Dependency note
---------------
This script requires Kornia for RGB<->Lab conversion.
"""
...
def topk_score_loss_from_raw(raw, topk: int):
    ...
    Minimizes mean(top-k logits) to suppress detections.
    """
...
def save_deltaL_artifacts(
    ...
    """
    Saves:
      - <tag>_iter_XXXX.npy : raw deltaL in L-units (masked)
      - <tag>_iter_XXXX.png : visualization
    """
    ensure_dir(out_dir)
...
def main():
    ap = argparse.ArgumentParser(
        description="Lab-L-only masked I-FGSM attack with character exclusion masking, EOT, and robustness evaluation for YOLOv8 license-plate detection."
    )
    ...
    ap.add_argument("--out_dir", type=str, default="outputs/adv_lab_l_char_mask_eot", help="Directory where outputs will be saved.")

    ap.add_argument("--imgsz", type=int, default=640, help="Letterbox image size used for the detector.")

    ap.add_argument("--eps", type=float, default=32 / 255,
                    help="Maximum perturbation budget in normalized [0,1] scale; internally converted to Lab-L units via *100.")
    ap.add_argument("--steps", type=int, default=300, help="Number of attack optimization steps.")
    ap.add_argument("--alpha", type=float, default=12 / 255,
    ...
    ap.add_argument("--earlystop_conf", type=float, default=0.25, help="Confidence threshold used for detector-based early stopping.")

    ap.add_argument("--device", type=str, default="auto", help="Computation device: auto, cpu, cuda, cuda:0, etc.")

    ap.add_argument("--save_plate_patch", action=argparse.BooleanOptionalAction, default=True,
    ...
    ap.add_argument("--debug_heatmap", action=argparse.BooleanOptionalAction, default=True,
    ...
    ap.add_argument("--save_char_mask", action=argparse.BooleanOptionalAction, default=True,
                    help="Whether to save the character mask and final plate attack mask.")
    ...
    ap.add_argument("--save_lum_every", type=int, default=25,
    ...
    ap.add_argument("--save_eot_samples", action=argparse.BooleanOptionalAction, default=True,
    ...
    ap.add_argument("--robust_save_max", type=int, default=10,
    ...
    alpha_base = args.alpha if args.alpha is not None else 2.0 * (args.eps / max(args.steps, 1))
    alpha_L = float(alpha_base) * 100.0
    ...
    for img_path in img_files:
        ...
        try:
            for t in range(args.steps):
                ...
                rgb_adv_full = kc.lab_to_rgb(lab_adv).clamp(0, 1)
                ...
                if (t % max(args.log_every, 1) == 0) or (t == args.steps - 1):
                    ...
                    lmin = float(Ls.min().detach().cpu().item())
                    lmax = float(Ls.max().detach().cpu().item())
                    sat = (deltaL.abs() >= (eps_L - 1e-12)).float().mean().item() * 100.0
                    dinf = deltaL.abs().max().item()
                    print(
                    ...
                    print(f"    deltaL | Linf(L-units)={dinf:.4f} / eps_L={eps_L:.4f} | sat%={sat:.2f}")
                ...
                with torch.no_grad():
                    deltaL = (deltaL - alpha_L * deltaL.grad.sign()).clamp(-eps_L, eps_L)
                    deltaL = (deltaL * plate_mask).detach().requires_grad_(True)
        ...
        with open(os.path.join(per_dir, "summary.txt"), "w", encoding="utf-8") as f:
            ...
            f.write(f"alpha(rgb-scale): {alpha_base} -> alpha_L: {alpha_L}\n")
